...
import com.hqjy.transfer.common.base.utils.RegularUtils;
...
import com.hqjy.transfer.crm.modules.sys.dao.SysUserDao;
...
import static com.hqjy.transfer.crm.common.utils.ShiroUtils.getUserId;
...
/*    @Autowired
    ...
    public int updatePassword(HashMap<String, Object> param) {
        ...
        SysUserEntity sysUserOld = baseDao.findOne(getUserId());
        //旧密码错误
    ...
    public int updateUserAvatar(MultipartFile file) {
        if (!file.isEmpty()) {
            String oldKey = Optional.ofNullable(baseDao.findOne(getUserId())).map(SysUserEntity::getAvatar).orElse(null);
            //aliyun目录
    ...
    public void updateUserRedisInfo() {
        ...
        LoginUserDTO userDTO = shiroService.findByUserId(getUserId());
