...
public class TransferGenWayController {
    ...
    @ApiOperation(value = "修改推广方式", notes = "输入参数：\n" +
            "参数说明：\n" +
            " 【推广方式id:wayId】,【推广平台Id:sourceId】,【排序号:seq】, 【状态( 1:正常; 0:禁用):status】\n" +
            "示例：\n" +
            "{\n" +
            "  \"wayId\": 1,\n" +
            "  \"sourceId\": 2,\n" +
    ...
    @SysLog("修改推广方式")
    @PutMapping
    public R update(@Validated(RestfulValid.PUT.class) @RequestBody TransferWaySourceEntity transferWaySourceEntity) {
        int count = transferWaySourceService.update(transferWaySourceEntity);
        if (count > 0) {
            return R.ok();
        }
